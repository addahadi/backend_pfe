...
import { NotFoundError, ConflictError, AuthError } from '../../utils/AppError.js';
...
export const createSubscription = async ({ userId, planId }) => {
  return await sql.begin(async (tx) => {
    ...
    if (existing.length) {
      throw new ConflictError('You already have an active subscription to this plan', 'لديك بالفعل اشتراك نشط في هذه الخطة');
    }
...
export const createSubscriptionLegacy = async ({ userId, planId }) => {
  return await sql.begin(async (tx) => {
    ...
    if (!plan.length) {
      throw new Error('Plan not found');  // legacy path
    }
...
export const getActiveSubscription = async (userId) => {
  ...
  if (!result.length) {
    throw new NotFoundError('No active subscription found', 'لا يوجد اشتراك نشط');
  }
...
export const validateSwitchToken = (token) => {
  try {
    ...
    if (err.name === 'TokenExpiredError') {
      throw new AuthError('Confirmation token has expired', 'انتهت صلاحية رمز التأكيد');
    }
    throw new AuthError('Invalid confirmation token', 'رمز التأكيد غير صالح');
  }
};
